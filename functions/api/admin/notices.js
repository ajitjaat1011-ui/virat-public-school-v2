...
import { requireUser, checkOrigin, audit, sanitizeHtml, slugify, cuid } from '../../lib/auth.js';

export async function onRequestGet(context) {
  const { env, request } = context;
  const { user, error } = await requireUser(request, env);
  if (error) return errorResponse(error, 401);
  const { results } = await db(env).prepare('SELECT * FROM notices WHERE deleted_at IS NULL ORDER BY publish_date DESC LIMIT 200').all();
...
export async function onRequestPost(context) {
  ...
  const existing = await db(env).prepare('SELECT id FROM notices WHERE slug = ?1').bind(slug).first();
